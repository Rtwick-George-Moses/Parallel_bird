...
def save_pool():
    for xi in range(total_models):
        current_pool[xi].save_weights("model3/SavedModels/model_new" + str(xi) + ".keras")
...
def showGameOverScreen(crashInfo):
    ...
    if  generation > max_g:
            ...
            holder = toc - tic
            time_hold = loadtxt('time.csv', delimiter=',')
            time_hold[2]= holder
            # save to csv file
            print("done_for this-round-mod3")
            print("no_of_generations = "+str(max_g))
            savetxt('time.csv', time_hold, delimiter=',')
    run()
    return
...
if load_saved_pool:
    for i in range(total_models):
        current_pool[i].load_weights("model3/SavedModels/model_new"+str(i)+".keras")
...
def run( ):
    global max_g
    global tic
    generations = loadtxt('generations.csv', delimiter=',')
    max_g = generations[2]
    if max_g == -1:
        pygame.quit()
        sys.exit()
